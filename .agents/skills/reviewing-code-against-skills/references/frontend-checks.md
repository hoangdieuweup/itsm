...
## Cross-module imports & Proxy Re-exports
...
## Hardcoded Strings & Missing Constants
Flag any raw string literals used for auth statuses, roles, resources, actions, or permissions across components:
...
- `<Can I="read" a="user">` → MUST use `<Can I={ACTIONS.READ} a={RESOURCES.USER}>`.
- `<RequirePermission resource="user" action="read">` → MUST use constants `RESOURCES.USER` and `ACTIONS.READ`.
- Role names (e.g. `"admin"`) → MUST use `SYSTEM_ROLE_NAMES.ADMIN` or `isProtectedAdminRole()`.

## Missing SSR prefetch & Unconditional SSR Prefetch
...
- Raw string literals used inside `<Can>` or `<RequirePermission>` instead of constants from `shared/constants/permissions.ts`.

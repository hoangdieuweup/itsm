...
## Entities vs Modules Division of Responsibility

When an entity and module share a domain name (e.g. `entities/role` and `modules/roles`):

| Concern | `entities/<name>/` (Core Entity) | `modules/<feature>/` (Feature Module) |
|---|---|---|
| **Schema & Types** | `model/schema.ts` (`roleSchema`, `type Role`) — **Single Source of Truth** | **NEVER duplicate**. Do not create a second `schema.ts`. |
| **Query Keys** | `api/query-keys.ts` (`rolesKeys.all`, `.list()`, `.detail()`) | Imports `rolesKeys` from `@/entities/role`. |
| **Read Operations** | `api/fetchers.ts` (`fetchRoles`), `hooks/use-roles.ts` | Uses `useRoles` from `@/entities/role`. |
| **Mutations / Actions** | None (pure domain reads only) | `hooks/use-create-role.ts`, `use-update-role.ts`, `use-delete-role.ts` |
| **UI Components** | Atomic UI badges/avatars (`role-badge.tsx`) | Feature views, tables, dialogs (`role-form-dialog.tsx`, `roles-page-content.tsx`) |
| **Public API (`index.ts`)** | Exports schemas, types, read hooks, entity badges | Exports page components, dialogs, mutation hooks |

### Hard Rule: No Proxy Re-exports

Never create dummy proxy files that merely re-export from `entities/`:
```ts
// ❌ WRONG: modules/roles/model/schema.ts
export * from "@/entities/role/model/schema"; // FORBIDDEN: proxy re-export file
```
**Fix:** Delete the proxy file completely. Callers in `modules/roles/ui/` must import directly from `@/entities/role`:
```ts
// ✅ CORRECT: direct root import

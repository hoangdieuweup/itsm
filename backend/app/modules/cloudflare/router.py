...
from app.modules.cloudflare.services.accounts.create_account import CreateCloudflareAccount
from app.modules.cloudflare.services.accounts.delete_account import DeleteCloudflareAccount
from app.modules.cloudflare.services.accounts.list_visible_accounts import ListVisibleCloudflareAccounts
from app.modules.cloudflare.services.accounts.list_zones import ListZones
from app.modules.cloudflare.services.accounts.reveal_token import RevealCloudflareAccountToken
from app.modules.cloudflare.services.accounts.test_connection import TestCloudflareAccountConnection
from app.modules.cloudflare.services.accounts.update_account import UpdateCloudflareAccount
from app.modules.cloudflare.services.configs.create_config import CreateCloudflareConfig
from app.modules.cloudflare.services.configs.delete_config import DeleteCloudflareConfig
from app.modules.cloudflare.services.configs.update_config import UpdateCloudflareConfig
from app.modules.cloudflare.services.dns.create_account_dns_record import CreateAccountDnsRecord
from app.modules.cloudflare.services.dns.create_dns_record import CreateDnsRecord
from app.modules.cloudflare.services.dns.delete_account_dns_record import DeleteAccountDnsRecord
from app.modules.cloudflare.services.dns.delete_dns_record import DeleteDnsRecord
from app.modules.cloudflare.services.dns.list_account_dns_records import ListAccountDnsRecords
from app.modules.cloudflare.services.dns.list_dns_records import ListDnsRecords
from app.modules.cloudflare.services.dns.sync_dns_records import SyncDnsRecords
from app.modules.cloudflare.services.dns.update_account_dns_record import UpdateAccountDnsRecord
from app.modules.cloudflare.services.dns.update_dns_record import UpdateDnsRecord
from app.modules.cloudflare.services.managers.assign_manager import AssignCloudflareAccountManager
from app.modules.cloudflare.services.managers.list_account_managers import ListCloudflareAccountManagers
from app.modules.cloudflare.services.managers.remove_manager import RemoveCloudflareAccountManager
from app.modules.cloudflare.services.managers.update_manager import UpdateCloudflareAccountManager
from app.modules.cloudflare.services.traffic.get_traffic_stats import GetCloudflareTrafficStats
from app.modules.cloudflare.services.tunnel_hostnames.add_tunnel_hostname import AddTunnelHostname
from app.modules.cloudflare.services.tunnel_hostnames.list_tunnel_hostnames import ListTunnelHostnames
from app.modules.cloudflare.services.tunnel_hostnames.remove_tunnel_hostname import RemoveTunnelHostname
from app.modules.cloudflare.services.tunnel_hostnames.update_tunnel_hostname import UpdateTunnelHostname
from app.modules.cloudflare.services.tunnels.create_account_tunnel import CreateAccountTunnel
from app.modules.cloudflare.services.tunnels.create_tunnel import CreateCloudflareTunnel
from app.modules.cloudflare.services.tunnels.delete_account_tunnel import DeleteAccountTunnel
from app.modules.cloudflare.services.tunnels.delete_tunnel import DeleteCloudflareTunnel
from app.modules.cloudflare.services.tunnels.list_account_tunnels import ListAccountTunnels
from app.modules.cloudflare.services.tunnels.list_tunnels import ListTunnels
from app.modules.cloudflare.services.tunnels.refresh_tunnel_status import RefreshTunnelStatus
...
async def create_cloudflare_account(
    ...
    user: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_ACCOUNT, RbacActions.CREATE)),
...
async def list_cloudflare_accounts(
    ...
    _user: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_ACCOUNT, RbacActions.READ)),
...
async def update_cloudflare_account(
    ...
    _l1: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_ACCOUNT, RbacActions.UPDATE)),
...
async def delete_cloudflare_account(
    ...
    _l1: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_ACCOUNT, RbacActions.DELETE)),
...
async def test_cloudflare_account_connection(
    ...
    _l1: UserRead = Depends(
        require_permission(RbacResources.CLOUDFLARE_ACCOUNT, RbacActions.TEST_CONNECTION)
    ),
...
async def reveal_cloudflare_account_token(
    ...
    _l1: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_ACCOUNT, RbacActions.REVEAL_TOKEN)),
    grant: AccountAccessGrant = Depends(require_account_access(AccessLevel.OWNER)),
...
async def list_cloudflare_account_managers(
    ...
    _l1: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_MANAGER, RbacActions.READ)),
    _grant: AccountAccessGrant = Depends(require_account_access(AccessLevel.VIEWER)),
...
async def assign_cloudflare_account_manager(
    ...
    _l1: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_MANAGER, RbacActions.MANAGE)),
    grant: AccountAccessGrant = Depends(require_account_access(AccessLevel.OWNER)),
) -> ApiResponse[None]:
...
async def create_cloudflare_config(
    ...
    user: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_CONFIG, RbacActions.MANAGE)),
...
async def get_cloudflare_config(
    ...
    _grant: AccountAccessGrant = Depends(
        require_cloudflare_environment_access(
            RbacResources.CLOUDFLARE_CONFIG,
            RbacResources.PROJECT_CLOUDFLARE_CONFIG,
            RbacActions.READ,
            AccessLevel.VIEWER,
        )
    ),
...
async def update_cloudflare_config(
    ...
    _l1: UserRead = Depends(require_permission(RbacResources.CLOUDFLARE_CONFIG, RbacActions.MANAGE)),
...
async def list_environment_dns_records(
    ...
    _grant: AccountAccessGrant = Depends(
        require_cloudflare_environment_access(
            RbacResources.CLOUDFLARE_DNS,
            RbacResources.PROJECT_CLOUDFLARE_DNS,
            RbacActions.READ,
            AccessLevel.VIEWER,
        )
    ),
...
async def update_environment_dns_record(
    ...
    record = await use_case.execute(
        environment_id, record_id, body.content, body.priority, body.proxied, body.ttl, actor=grant.user
    )
    return ApiResponse[DnsRecordRead](success=True, data=record)
...
async def delete_environment_dns_record(
    ...
    await use_case.execute(environment_id, record_id, actor=grant.user)
    return ApiResponse[None](success=True)
...
async def list_environment_tunnels(
    ...
    _grant: AccountAccessGrant = Depends(
        require_cloudflare_environment_access(
            RbacResources.CLOUDFLARE_TUNNEL,
            RbacResources.PROJECT_CLOUDFLARE_TUNNEL,
            RbacActions.READ,
            AccessLevel.VIEWER,
        )
    ),
...
async def create_environment_tunnel(
    ...
    tunnel, token = await use_case.execute(environment_id, body.name, actor=grant.user)
...
async def delete_environment_tunnel(
    ...
    commonly serves several unrelated projects at once (no environment_id
    column at all), so deleting one from inside a single project would
    silently take down every other project it serves. Only genuine
    Cloudflare account access can do this."""
    await use_case.execute(environment_id, tunnel_id, actor=grant.user)
    return ApiResponse[None](success=True)
...
async def reveal_environment_tunnel_token(
    ...
    grant: AccountAccessGrant = Depends(
        require_cloudflare_environment_access(
            RbacResources.CLOUDFLARE_TUNNEL, None, RbacActions.REVEAL_TOKEN, AccessLevel.EDITOR
        )
    ),
    ...
    project_resource=None: reveals a live connector token for a tunnel that
    may serve several other projects — account access only, no project role."""
    token = await use_case.execute(environment_id, tunnel_id, actor=grant.user)
...
async def list_environment_tunnel_hostnames(
    ...
    _grant: AccountAccessGrant = Depends(
        require_cloudflare_environment_access(
            RbacResources.CLOUDFLARE_HOSTNAME,
            RbacResources.PROJECT_CLOUDFLARE_HOSTNAME,
            RbacActions.READ,
            AccessLevel.VIEWER,
        )
    ),
...
async def add_environment_tunnel_hostname(
    ...
    hostname = await use_case.execute(
        environment_id, tunnel_id, body.hostname, body.service, actor=grant.user
    )
...
async def update_environment_tunnel_hostname(
    ...
    hostname = await use_case.execute(environment_id, tunnel_id, hostname_id, body.service, actor=grant.user)
    return ApiResponse[TunnelPublicHostnameRead](success=True, data=hostname)
...
async def remove_environment_tunnel_hostname(
    ...
    await use_case.execute(environment_id, tunnel_id, hostname_id, actor=grant.user)
    return ApiResponse[None](success=True)
